    ...
    <div class="w3-panel w3-padding-large w3-card-4 w3-light-grey">
        ...
        <p class="w3-large">
            <div class="w3-code cssHigh notranslate">
				...
				$sql  = "SELECT t.codTurma, t.CodFuncionario, t.codhabi, t.ano, t.semestre FROM Turma as t WHERE codTurma = $id ";
				...
				if ($result = mysqli_query($conn, $sql)) {
					if (mysqli_num_rows($result) > 0) {
						...
						//Obtém lista de habilidades
						if ($result = mysqli_query($conn, $sqlD)) {
							while ($row = mysqli_fetch_assoc($result)) {
								$selected = "";
								if ($row['Codhabilidade'] == $codhabi)
									$selected = "selected";
							   array_push($optionshabi, "\t\t\t<option " . $selected . " value='". $row["Codhabilidade"]."'>".$row["Nomehabi"]."</option>\n");
							}
						}
						//Obtém lista de Funcionarios
						if ($result = mysqli_query($conn, $sqlP)) {
							while ($row = mysqli_fetch_assoc($result)) {
								$selected = "";
								if ($row['CodFuncionario'] == $codfunc)
									$selected = "selected";
								array_push($optionsfunc, "\t\t\t<option " . $selected . " value='". $row["CodFuncionario"]."'>".$row["Nome"]."</option>\n");
							}
						}
